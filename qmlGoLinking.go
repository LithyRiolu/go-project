...
type QmlBridge struct {
  ...
  _ func()                           `slot:"copyAddressClicked"`
	...
	_ func(ccxAmount string) string `slot:"getTransferAmountUSD"`
...
func connectQMLToGOFunctions() {
	...
	qmlBridge.ConnectGetTransferAmountUSD(func(ccxAmount string) string {
		return amountStringUSDToCCX(ccxAmount)
	})
	...
	qmlBridge.ConnectClickedButtonImport(func(filenameWallet string, passwordWallet string, privateViewKey string, privateSpendKey string, confirmPasswordWallet string, scanHeight string) {
		go func() {
			importWalletWithWalletInfo(filenameWallet, passwordWallet, confirmPasswordWallet, privateViewKey, privateSpendKey, scanHeight)
		}()
	})
