...
var (
	...
	stringBackupKeys            = ""
	ccxToUSD                    float64 // USD value for 1 CCX
...
func main() {
	...
	if isPlatformDarwin {
		...
		pathToAppFolder := pathToHomeDir + "/Library/Application Support/Stash"
	...
	log.WithField("version", stashVersion).Info("Application started")
	...
	app.SetWindowIcon(gui.NewQIcon5("qrc:/frontend/Images/icon.png"))

	quickcontrols2.QQuickStyle_SetStyle("material")

	engine := qml.NewQQmlApplicationEngine(nil)
	engine.Load(core.NewQUrl3("frontend/main.qml", 0))
	...
	go func() {
		newVersionAvailable, urlNewVersion = checkIfNewReleaseAvailableOnGithub(stashVersion)
	...
	walletdmanager.GracefullyQuitConceald()
}
...
func getAndDisplayBalances() {
	...
	if err == nil {
		...
		balanceUSD := walletTotalBalance * ccxToUSD
	...
	if err == nil {
		...
		if needFullUpdate {
			...
			for index, transfer := range transfers {
				...
				amountString += " CCX (fee: " + strconv.FormatFloat(transfer.Fee, 'f', 2, 64) + ")"
...
func transfer(transferAddress string, transferAmount string, transferPaymentID string, transferFee string) {
	...
	qmlBridge.FinishedSendingTransaction()
	qmlBridge.DisplayPopup("CCX sent successfully", 4000)
}
...
func importWalletWithWalletInfo(filenameWallet string, passwordWallet string, confirmPasswordWallet string, privateViewKey string, privateSpendKey string, scanHeight string) bool {

	err := walletdmanager.CreateWallet(filenameWallet, passwordWallet, confirmPasswordWallet, privateViewKey, privateSpendKey, scanHeight)
	if err != nil {
...
func showWalletPrivateInfo() {
	...
	privateViewKey, privateSpendKey, err := walletdmanager.GetPrivateKeys()
	if err != nil {
...
func requestRateCCX() {
	response, err := http.Get(urlCryptoCompareCCX)
	...
	if err != nil {
		...
		if err != nil {
			...
			if err := json.Unmarshal(b, &resultInterface); err != nil {
				...
				ccxToUSD = resultsMap["USD"].(float64)
			}
		}
	}
}
...
func amountStringUSDToCCX(ccxAmountString string) string {
	ccxAmount, err := strconv.ParseFloat(ccxAmountString, 64)
	if err != nil || ccxAmount <= 0 || ccxToUSD == 0 {
		return ""
	}
	amountUSD := ccxAmount * ccxToUSD
